 ...
 * @version    2.7
 * @$Id: lightbox.inline.php 937943 2014-06-24 17:06:54Z dzappone $
 * @$URL: http://plugins.svn.wordpress.org/lightbox-plus/trunk/admin/lightbox.inline.php $
 */

$g_lbp_inline_options = $this->getAdminOptions( $this->lightboxOptionsInlineName );
?>
				...
				<table class="wp-list-table widefat">
					...
					<tr>
						...
						<th style="text-align:center;">
							<b><?php _e( 'Link Class', 'lightboxplus' ); ?></b><br /><b><?php _e( 'Content ID', 'lightboxplus' ); ?></b>
						</th>
						<th style="text-align:center;">
					...
					for ( $i = 1; $i <= $g_lbp_options['inline_num']; $i ++ ) {
//                                    $inline_links            = array();
//                                    $inline_hrefs            = array();
						...
						<tr <?php if ( $i % 2 == 0 ) {
							...
							<td align="center">
								<input type="text" size="15" name="inline_link[<?php echo $i; ?>]" id="inline_link_<?php echo $i; ?>" value="<?php if ( empty( $inline_links[ $i ] ) ) {
									...
									echo $inline_links[ $i ];
								} ?>" /><br /><input type="text" size="15" name="inline_href[<?php echo $i; ?>]" id="inline_href_<?php echo $i; ?>" value="<?php if ( empty( $inline_hrefs[ $i ] ) ) {
							...
							<td align="center">
								<select name="inline_transition[<?php echo $i; ?>]" id="inline_transition_<?php echo $i; ?>">
									...
									if ( isset( $inline_transitions[ $i ] ) && $inline_transitions[ $i ] == 'fade' ) {
										echo ' selected="selected"';
									}
									?>>Fade
									</option>
									<option value="none"<?php if ( isset( $inline_transitions[ $i ] ) && $inline_transitions[ $i ] == 'none' ) {
										echo ' selected="selected"';
									}
									?>>None
									</option>
								</select><br />
								<select name="inline_speed[<?php echo $i; ?>]" id="inline_speed_<?php echo $i; ?>">
									<?php
									for ( $j = 0; $j <= 5001; ) {
										?>
										<option value="<?php echo $j; ?>"<?php
										if ( isset( $inline_speeds[ $i ] ) && $inline_speeds[ $i ] == strval( $j ) ) {
											echo ' selected="selected"';
										} ?>><?php echo $j; ?></option>
										<?php
										if ( $j >= 2000 ) {
							...
							<td align="center">
								...
								} ?>" /><input type="text" size="5" name="inline_position_bottom_<?php echo $i; ?>" id="inline_position_bottom_<?php echo $i; ?>" value="<?php if ( empty( $inline_position_bottoms[ $i ] ) ) {
									echo '';
								} else {
							...
							<td align="center">
								...
								<input type="checkbox" name="inline_fixed[<?php echo $i; ?>]" id="inline_fixed_<?php echo $i; ?>" value="1"<?php if ( isset( $inline_fixeds[ $i ] ) && $inline_fixeds[ $i ] == 1 ) {
									echo ' checked="checked"';
								} ?> />
							</td>
							<td align="center">
								<input type="hidden" name="inline_open[<?php echo $i; ?>]" value="0" />
								<input type="checkbox" name="inline_open[<?php echo $i; ?>]" id="inline_open_<?php echo $i; ?>" value="1"<?php if ( isset( $inline_opens[ $i ] ) && $inline_opens[ $i ] == 1 ) {
									echo ' checked="checked"';
								} ?> />
							</td>
							<td align="center">

								<select name="inline_opacity[<?php echo $i; ?>]" id="inline_opacity_<?php echo $i; ?>">
									<?php
									for ( $j = 0; $j <= 1.01; $j = $j + .05 ) {
										?>
										<option value="<?php echo $j; ?>"<?php if ( isset( $inline_opacitys[ $i ] ) && $inline_opacitys[ $i ] == strval( $j ) ) {
											echo ' selected="selected"';
										} ?>><?php echo( $j * 100 ); ?>%
										</option>
									<?php
						...
						<td>
							...
							<div id="lbp_for_inline_tip">
								...
								<div class="ui-state-highlight ui-corner-all" style="margin-top: 20px; padding: 0 .7em;">
									<h5>
										<span class="ui-icon ui-icon-info" style="float: left; margin-right: .3em;"></span><?php _e( 'Example', 'lightboxplus' ) ?>
									</h5>
					...
					<tr valign="top">
						<td>
							...
							<p class="inline_link_test_item">
								<a class="<?php echo $inline_links[1]; ?>" href="#"><?php _e( 'Inline Content Test including form', "lightboxplus" ); ?></a>
							</p>
