...
for ( $i = 1; $i <= $g_lbp_base_options['inline_num']; $i ++ ) {
	...
	<tr<?php if ( $i % 2 == 0 ) {
		...
		<td align="center">
			...
			} ?> title="<?php _e( "If checked, inline lightbox $i will always be displayed in a fixed position within the viewport. In other words it will stay within the viewport while scrolling on the page. This is unlike the default absolute positioning relative to the document. DEFAULT: Unchecked", "lightboxplus" ) ?>" />
		</td>
		<td align="center">
			...
			<input type="checkbox" name="inline_open[<?php echo $i; ?>]" id="inline_open_<?php echo $i; ?>" value="1"<?php if ( isset( $inline_opens[ $i ] ) && $inline_opens[ $i ] == 1 ) {
				echo ' checked="checked"';
			} ?>  title="<?php _e( "If checked, inline lightbox $i will automatically open when the page is loaded. DEFAULT: Unchecked", "lightboxplus" ) ?>" />
		</td>
		<td align="center">
